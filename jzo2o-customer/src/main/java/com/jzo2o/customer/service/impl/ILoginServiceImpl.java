...
import com.jzo2o.customer.mapper.UserMapper;
...
import com.jzo2o.customer.handler.jwtUtils;
...
public class ILoginServiceImpl implements ILoginService {
    ...
    @Override
    public Result<LoginResDTO> login(Long number, String password) {
        // 查询数据库拿到用户密码
        String userPassword = userMapper.getPasswordByNumber();
        ...
        if (md5Password.equals(userPassword)){
            // 密码正确
            //构建token
            String token = jwtUtils.createToken(number,password);
            return Result.ok(new LoginResDTO(token));
        }else {
            // 密码错误
