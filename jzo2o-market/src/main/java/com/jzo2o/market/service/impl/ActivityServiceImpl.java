...
//        long sortBy = DateUtils.toEpochMilli(activity.getDistributeStartTime()) * MILLION + activity.getId() % MILLION;
    ...
    public void preHeat() {
        ...
        List<Activity> list = lambdaQuery()
                ...
                .in(Activity::getStatus, Arrays.asList(NO_DISTRIBUTE.getStatus(), NO_DISTRIBUTE.getStatus()))
                .orderByDesc(Activity::getDistributeStartTime)
                .list();
        if (CollUtils.isEmpty(list)){
            list = CollUtils.emptyList();
        }
        List<SeizeCouponInfoResDTO> seizeCouponInfoResDTOList = BeanUtils.copyToList(list, SeizeCouponInfoResDTO.class);
        String jsonStr = JsonUtils.toJsonStr(seizeCouponInfoResDTOList);
        // 存入redis
        redisTemplate.opsForValue().set(ACTIVITY_CACHE_LIST,jsonStr);

        // 活动库存预热
        // 未开启的活动直接保存
        list.stream().filter(item -> NO_DISTRIBUTE.equals(getAccurateStatus(item.getStatus(),item.getDistributeStartTime(),item.getDistributeEndTime())))
                .forEach(item -> {
                    String key = String.format(COUPON_RESOURCE_STOCK,item.getId()%10);
                    // 同步到redis
                    redisTemplate.opsForHash().put(key,item.getId(),item.getTotalNum());
                });
        // 开启的活动不存在才保存
        list.stream().filter(item -> DISTRIBUTING.equals(getAccurateStatus(item.getStatus(),item.getDistributeStartTime(),item.getDistributeEndTime())))
                .forEach(item -> {
                    String key = String.format(COUPON_RESOURCE_STOCK,item.getId()%10);
                    // 同步到redis
                    redisTemplate.opsForHash().putIfAbsent(key,item.getId(),item.getTotalNum());
                });
    }

    /**
     * 用户端抢券列表分页接口
     * @param tabType 页面tab类型，1：疯抢中，2：即将开始
     * @return
     */
    @Override
    public List<SeizeCouponInfoResDTO> queryForCache(Integer tabType) {
        // 页面tab类型，1：疯抢中，2：即将开始
        Integer getStatus = tabType == 1 ? DISTRIBUTING.getStatus(): NO_DISTRIBUTE.getStatus();
        // 查询redis拿到活动列表
        Object jsonString = redisTemplate.opsForValue().get(ACTIVITY_CACHE_LIST);
        if (ObjectUtils.isNull(jsonString)){
            return CollUtils.emptyList();
        }
        List<SeizeCouponInfoResDTO> list = JsonUtils.toList(jsonString.toString(), SeizeCouponInfoResDTO.class);
        // 活动筛选及数据处理
        List<SeizeCouponInfoResDTO> resDTOList = list.stream().filter(item -> getStatus.equals(getAccurateStatus(item.getStatus(), item.getDistributeStartTime(), item.getDistributeEndTime())))
                .peek(item -> {
     ...
     * @return
     */
    public Integer getAccurateStatus(Integer status, LocalDateTime distributeStartTime,LocalDateTime distributeEndTime){
        LocalDateTime now = LocalDateTime.now();
        if (distributeEndTime.isBefore(now)){
            // 活动已经结束
            return LOSE_EFFICACY.getStatus();
        }else if (status == NO_DISTRIBUTE.getStatus() && distributeStartTime.isBefore(now)){
            // 活动已经开始了
     ...
     * @return
     */
    public ActivityInfoResDTO getActivityInfoByCache(Long activityId){
        // 根据活动id查询redis获得活动信息
        Object objectString = redisTemplate.opsForValue().get(ACTIVITY_CACHE_LIST);
        if (ObjectUtils.isNull(objectString)){
            return null;
        }
        List<ActivityInfoResDTO> list = JsonUtils.toList(objectString.toString(), ActivityInfoResDTO.class);
        if (CollUtils.isEmpty(list)){
            return null;
        }
        // 过滤返回
        return list.stream().filter(item -> item.getId().equals(activityId)).findFirst().orElse(null);
    }
}
